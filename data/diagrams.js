...
// ---------------------------------------------------------------
export function operatorPrecedenceDiagram() {
  const rows = [
    ['1', '() ', 'Grouping — always evaluated first'],
    ['2', '**', 'Exponentiation'],
    ...
    ['9', '= += -= ...', 'Assignment (evaluated last, right to left)'],
  ];
  let els = '';
  rows.forEach((r, i) => {
    const y = 30 + i * 30;
    els += box(30, y, 34, 22, r[0], { fontSize: 11, stroke: BLUE_BORDER, fill: BLUE_LIGHT, labelColor: BLUE, rx: 5 });
    els += label(150, y + 15, r[1], { anchor: 'middle', size: 13, color: INK, family: "'JetBrains Mono',monospace", weight: '600' });
    els += label(280, y + 15, r[2], { anchor: 'start', size: 11.5, color: MUTED });
  });
  return svg('0 0 640 300', els);
}

export function typeCoercionDiagram() {
  return svg('0 0 640 220', `
    ${arrowMarkerDefs('tc')}
    ${box(40, 90, 150, 50, '"5" (string)', { stroke: BLUE_BORDER, fill: BLUE_LIGHT, labelColor: BLUE })}
    ${box(245, 90, 150, 50, '5 (number)', { stroke: BLUE, fill: '#FFFFFF' })}
    ${box(450, 90, 150, 50, 'true (boolean)', { stroke: ACCENT_GREEN, fill: '#FFFFFF', labelColor: ACCENT_GREEN })}
    ${hArrow(190, 243, 115, 'tc')}
    ${hArrow(395, 448, 115, 'tc')}
    ${label(217, 100, 'Number("5")', { size: 10.5, italic: true })}
    ${label(422, 100, 'Boolean(5)', { size: 10.5, italic: true })}
    ${label(320, 190, 'JavaScript converts between types automatically in mixed expressions —', { size: 11.5, color: MUTED })}
    ${label(320, 206, 'implicit coercion follows fixed rules, not guesswork.', { size: 11.5, color: MUTED })}
  `);
}

export function shortCircuitDiagram() {
  return svg('0 0 640 210', `
    ${arrowMarkerDefs('sc')}
    ${box(30, 70, 140, 50, 'left operand', { stroke: BLUE })}
    ${box(250, 30, 160, 50, '&&: left is falsy\n→ stop, return left', { stroke: ACCENT_RED, labelColor: ACCENT_RED, fontSize: 11 })}
    ${box(250, 110, 160, 50, '&&: left is truthy\n→ evaluate right', { stroke: ACCENT_GREEN, labelColor: ACCENT_GREEN, fontSize: 11 })}
    ${box(470, 110, 140, 50, 'right operand\nbecomes result', { stroke: BLUE, fontSize: 11 })}
    ${hArrow(170, 248, 55, 'sc')}
    ${hArrow(170, 248, 135, 'sc')}
    ${hArrow(410, 468, 135, 'sc')}
    ${label(320, 190, '|| and ?? follow the same short-circuit shape, with different truth tests.', { size: 11.5, color: MUTED })}
  `);
}
